<template lang="html">
  <div class="site-nav">
    <div class="site-nav-page">
      <div class="site-nav-menu fl" v-if="nickName">
        <span class="fl">欢迎，</span>
        <a href="javascript:;" class="site-nav-menu-login fl">{{nickName}}</a>
        <a href="javascript:;" @click="logout">退出登录</a>
      </div>
      <div class="site-nav-menu fl" v-else-if="logined">
        <a href="javascript:;" class="site-nav-menu-login fl" @click="$router.push({name: 'login'})">亲，请登录</a>
        <a href="javascript:;" class="site-nav-menu-reg fl" @click="$router.push({name: 'reg'})">免费注册</a>
      </div>
...
.site-nav {height:35px;background:#f5f5f5;border-bottom:1px solid #eee;position:relative;z-index:2;}
.site-nav a {color:#6C6C6C;font-size:12px;height:35px;line-height:35px;}
.site-nav-menu-login {margin-right:7px;}
.site-nav a.site-nav-menu-login {color:#f22e00!important;}
.site-nav a:hover {color:#F40;}
...
.site-nav-menu-container:hover {background:#FFF;}
.site-nav-menu-list {
  background:#FFF;
