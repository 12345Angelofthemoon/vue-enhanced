  ...
  <div>
    <header>
      ...
      <div class="search-container fr clearfix">
        <input type="text" class="search-input fl" @keydown.enter="applyFilter" v-model="form.q" placeholder="请输入要搜索的词">
...
export default class SearchHeader extends Vue {
  ...
  created(){
    this.form.q=this.$route.params.keyword;
    this.form.page=this.$route.params.page||1;
    this.form.sort=this.$route.query.sort||'';
    this.form.min_price=this.$route.query.min_price;
    this.form.max_price=this.$route.query.max_price;
